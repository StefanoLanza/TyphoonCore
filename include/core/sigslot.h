...
namespace detail
{
	...
	public:
		...
		constexpr slot_state() noexcept : m_connected(true), m_blocked(false)
		{
		}
...
private:
	...
	template <typename, typename...>
	friend class signal_base;
	scoped_connection(std::weak_ptr<detail::slot_state> s) noexcept : connection { std::move(s) }
	{
	}
};
...
namespace detail
{
	...
	public:
		...
		constexpr slot(F&& f, P&& p) : pmf { std::forward<F>(f) }, ptr { std::forward<P>(p) }
		{
		}
	...
	public:
		...
		template <typename F, typename P>
		constexpr slot_tracked(F&& f, P&& p) : func { std::forward<F>(f) }, ptr { std::forward<P>(p) }
		{
		}
	...
	public:
		...
		template <typename F, typename P>
		constexpr slot_pmf_tracked(F&& f, P&& p) : pmf { std::forward<F>(f) }, ptr { std::forward<P>(p) }
		{
		}
...
public:
	 ...
	 */
	template <typename Pmf, typename Ptr>
	std::enable_if_t<! trait::is_callable_v<arg_list, Pmf> && trait::is_weak_ptr_compatible_v<Ptr>, connection> connect(
	    Pmf&& pmf, Ptr&& ptr)
	{
		using trait::to_weak;
